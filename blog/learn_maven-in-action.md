...
Maven的生命周期是抽象的，意味着生命周期本身不做任何实际的工作,在Maven的设计中,实际的任务都交由插件来完成。Maven定义的声明周期和插件机制一方面保证了所有Maven项目有一致的构建标准，又通过默认插件简化和稳定了实际项目的构建，还提供了足够的扩展空间，用户可以通过配置现有插件或者自行编写插件来自定义构建行为。
...
**default生命周期**：定义了真正构建时所需执行的所有步骤，是所有生命周期中最核心的部分，包含如下阶段：
* validate
* initialize
* generate-sources
* process-sources
...
**site生命周期**：建立和发布项目站点，Maven能够基于POM所包含的信息,自动生成一个友好的站点,方便团队交流和发布项目信息。包含如下阶段：
* pre-site
* site
* post-site
* site-deploy
...
**mvn clean**：调用clean生命周期的clean阶段，实际执行的阶段为pre-clean和clean阶段。

**mvn test**：调用default生命周期的test阶段，实际执行的阶段为validate，initialize等，直到test的所有阶段。

**mvn clean install**：该命令结合两个生命周期，调用clean生命周期的clean阶段和default生命周期的install阶段,实际执行的阶段为clean生命周期的pre-clean、clean阶段,及default生命周期从validate至instal的所有阶段。
...
## Maven属性
通过\<properties>元素用户可以定义一个或多个Maven属性，然后在POM的其他地方使用\$
**{属性名称}** 的方式引用该属性，意义在于消除重复。
